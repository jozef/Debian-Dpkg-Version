...
to do the right thing when you compare Debian::Dpkg::Version object between
them.
...
=item my $v = Debian::Dpkg::Version->new($version, %opts)

Create a new Debian::Dpkg::Version object corresponding to the version indicated in
...
needs to be a Debian::Dpkg::Version, the other one can be anything provided that
...
sub comparison {
    my ($a, $b, $inverted) = @_;
    if (not ref($b) or not $b->isa("Debian::Dpkg::Version")) {
        $b = Debian::Dpkg::Version->new($b);
    }
    ($a, $b) = ($b, $a) if $inverted;
...
sub version_compare($$) {
    my ($a, $b) = @_;
    my $va = Debian::Dpkg::Version->new($a, check => 1) || error(_g("%s is not a valid version"), "$a");
    my $vb = Debian::Dpkg::Version->new($b, check => 1) || error(_g("%s is not a valid version"), "$b");
    return $va <=> $vb;
}
